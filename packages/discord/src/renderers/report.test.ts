...
describe('/report renderer architecture', () => {
  ...
  it('renders best execution highest-parse lines and biggest trouble pull-duration lines', () => {
    ...
    const biggestTrouble = getFieldValue(response, 'Biggest Trouble 👨‍🦼');

    expect(bestExecution).toContain('Highest Parse:');
    expect(bestExecution).toContain('DPS: Alyra - 95');
    expect(bestExecution).toContain('HPS: Alyra - 82');
    expect(bestExecution).toContain('DTPS: Bulwark - 77');
    expect(bestExecution).toContain('Highest Total DPS: Alyra - 40K/s');
    expect(bestExecution).toContain('Highest Total HPS: Alyra - 12K/s');
    expect(bestExecution).toContain('Highest Total DTPS: Bulwark - 18K/s');

    expect(biggestTrouble).toContain('Longest Pull: 3:20');
    expect(biggestTrouble).toContain('Shortest Pull: 2:10');
    expect(biggestTrouble).toContain('Highest Total DPS: Bulwark - 27K/s');
    expect(biggestTrouble).toContain('Highest Total HPS: Alyra - 15.5K/s');
    expect(biggestTrouble).toContain('Highest Total DTPS: Bulwark - 22K/s');
  });
  ...
  it('renders unavailable lines for missing highlight values and missing top-player blocks', () => {
    ...
    expect(bestExecution).toContain('Highest Total DTPS: unavailable');

    expect(getFieldValue(response, 'Highest Total DPS')).toBe('unavailable');
    expect(getFieldValue(response, 'Highest HPS')).toBe('unavailable');
    expect(getFieldValue(response, 'Highest DTPS')).toBe('unavailable');
    expect(getFieldValue(response, 'Most Interrupts')).toBe('unavailable');
  });
});
