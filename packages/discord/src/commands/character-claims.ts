...
export const getRequesterDiscordUserId = (interaction: DiscordInteraction): string | undefined =>
...
const requireExactCharacterInput = (
  interaction: DiscordInteraction,
): ReturnType<typeof buildExactCharacterClaimInput> =>
  buildExactCharacterClaimInput(interaction.data?.options);
...
export const handleMyCharactersCommand = async (
  ...
  return ephemeral(
    ['Your character claims:', ...claims.slice(0, 10).map(formatClaimLine)].join('\n'),
  );
};
