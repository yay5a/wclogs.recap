...
import { buildPublicRecapEmbed, buildRecapPreviewBody, parseRecapComponentCustomId } from "../renderers/embeds.js";
import { editOriginalInteractionResponse, safeEditOriginalInteractionResponse } from "../infrastructure/discord-api.js";
...
export const processReportRecapInteraction = async (
    ...
    const previewStateTtlMs = (options.previewStateTtlSeconds ?? DEFAULT_PREVIEW_STATE_TTL_SECONDS) * 1000;
    ...
    try {
        ...
        await safeEditOriginalInteractionResponse(applicationId, interactionToken, { flags: EPHEMERAL_MESSAGE_FLAG, content: userFacingContent });
    }
};
...
export const handleRecapComponentInteraction = async (
    ...
    if (!parsedCustomId) {
        return { type: 4, data: { content: "Unsupported interaction in MVP.", flags: 64 } };
    }
    ...
    const previewState = await options.recapPreviewStateService.consumeValidPreviewState({ reportCode, guildId });
    if (!previewState) {
