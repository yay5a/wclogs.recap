...
const getRequesterDiscordUserId = (interaction: DiscordInteraction): string | undefined =>
  interaction.member?.user?.id ?? interaction.user?.id;

const getRequesterPermissions = (interaction: DiscordInteraction): string | number | undefined =>
...
const normalizeOfficerUserIds = (userIds: readonly string[] | undefined): string[] => [
  ...new Set(
    (userIds ?? []).filter((userId) => typeof userId === 'string' && userId.trim().length > 0),
  ),
];

const requireGuild = (
  interaction: DiscordInteraction,
): { guildId: string } | { response: unknown } => {
...
const requireTargetUserId = (
  interaction: DiscordInteraction,
): { targetDiscordUserId: string } | { response: unknown } => {
...
const saveCompareOfficerUserIds = (
  guildConfigStore: GuildConfigStore,
  guildId: string,
  compareOfficerUserIds: string[],
): Promise<GuildConfig> => guildConfigStore.saveGuildConfig(guildId, { compareOfficerUserIds });

const addCompareOfficerUser = async (
  guildConfigStore: GuildConfigStore,
  guildId: string,
  ...
  const nextUserIds = currentUserIds.includes(discordUserId)
    ? currentUserIds
    : [...currentUserIds, discordUserId];
  if (
    rawCurrentUserIds.length === nextUserIds.length &&
    rawCurrentUserIds.every((userId, index) => userId === nextUserIds[index])
  ) {
    return current;
  }
...
const removeCompareOfficerUser = async (
  guildConfigStore: GuildConfigStore,
  guildId: string,
  discordUserId: string,
): Promise<{ removed: boolean; config: GuildConfig }> => {
  const current = await guildConfigStore.getGuildConfig(guildId);
  const currentUserIds = normalizeOfficerUserIds(current.compareOfficerUserIds);
  ...
  const config = guildConfigStore.removeCompareOfficerUser
    ? await guildConfigStore.removeCompareOfficerUser(guildId, discordUserId)
    : await saveCompareOfficerUserIds(guildConfigStore, guildId, nextUserIds);

  return { removed: true, config };
};

export const handleAddOfficerCommand = async (
  interaction: DiscordInteraction,
  ...
  await addCompareOfficerUser(
    options.guildConfigStore,
    context.guildId,
...
export const handleRemoveOfficerCommand = async (
  interaction: DiscordInteraction,
  options: HandleOptions,
): Promise<unknown> => {
  const context = requireGuild(interaction);
  ...
  const result = await removeCompareOfficerUser(
    options.guildConfigStore,
    context.guildId,
    target.targetDiscordUserId,
  );
  ...
  return ephemeral(
    result.removed
      ? `Officer removed: ${mentionUser(target.targetDiscordUserId)}.`
      : `${mentionUser(target.targetDiscordUserId)} was not configured as an officer.`,
  );
};

export const handleListOfficersCommand = async (
  interaction: DiscordInteraction,
  options: HandleOptions,
): Promise<unknown> => {
  const context = requireGuild(interaction);
