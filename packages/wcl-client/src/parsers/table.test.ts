...
const REPORT_WIDE_KILLS_PROBE_DIR = join(PROBE_ROOT, 'cNxRty7DWgPBLQT.table.report-wide.kills');
...
describe('table parser', () => {
  it('handles multiple table payload variants', () => {
    ...
    const deaths = parseTablePayload({ data: [{ id: 1, name: 'Alyra', deaths: 2 }] }, 'Deaths');
  ...
  it('treats explicit empty entries as valid empty payloads', () => {
    const warn = vi.fn();
    const parsed = parseTablePayloadDetailed({ data: { entries: [] } }, 'Deaths', warn);
  ...
  it('supports survivability payload shape', () => {
    ...
    const parsed = parseTablePayloadDetailed(
      {
        data: {
          actortotals: [{ id: 101, name: 'Alyra', survivability: 98.2 }],
        },
      },
      'Survivability',
      warn,
          ...
          entries: [
            {
              entries: [
                {
                  details: [{ id: 4, name: 'Shaman', total: 3 }],
                },
              ],
            },
          ],
...
const describeProbeFixtures = existsSync(REPORT_WIDE_KILLS_PROBE_DIR) ? describe : describe.skip;
...
describeProbeFixtures('report-wide kill probe payloads', () => {
  ...
  it('parses death event rows from probed payloads', () => {
    ...
    expect(parsed.entries.reduce((total, row) => total + row.value, 0)).toBe(19);
    expect(warn).not.toHaveBeenCalled();
  });
  ...
  it.each([
    ...
    (dataType, minimumTotal) => {
      ...
      expect(parsed.entries.reduce((total, row) => total + row.value, 0)).toBeGreaterThanOrEqual(
        minimumTotal,
      );
      expect(warn).not.toHaveBeenCalled();
    },
