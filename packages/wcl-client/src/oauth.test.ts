...
describe('WCL authorization-code OAuth', () => {
  ...
  it('exchanges a code with the configured redirect URI', async () => {
    ...
    expect(body.get('redirect_uri')).toBe('https://public.example.test/api/auth/wcl/callback');
  });
  ...
  it('derives expiry deterministically when expires_in is documented in the response', () => {
    ...
    expect(getWclTokenExpiresAt({ userAccessToken: 'access-token', expiresIn: 90 }, now))?.toEqual(
      new Date('2026-04-09T00:01:30.000Z'),
    );
