...
export async function resolveWclClientCredentialsToken(
  ...
  if (!clientId || !clientSecret) {
    throw new Error('Missing WCL client credentials. Provide WCL_CLIENT_ID and WCL_CLIENT_SECRET.');
  }

  const fetchImpl = options.fetchImpl ?? fetch;
  const auth = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');

  const response = await fetchImpl('https://www.warcraftlogs.com/oauth/token', {
    method: 'POST',
    headers: {
