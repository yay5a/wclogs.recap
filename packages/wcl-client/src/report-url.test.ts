...
describe('parseReportUrl', () => {
  it('parses report code from path', () => {
    const parsed = parseReportUrl('https://www.warcraftlogs.com/reports/abc123xyz4567890');
  ...
  it('parses a valid URL wrapped in angle brackets', () => {
    const parsed = parseReportUrl('<https://www.warcraftlogs.com/reports/abc123?fight=last>');
  ...
  it('fails with a clear error for invalid URL', () => {
    expect(() => parseReportUrl('not-a-url')).toThrow('Invalid Warcraft Logs report URL');
  });

  it('fails with a clear error when report code is missing', () => {
    expect(() => parseReportUrl('https://www.warcraftlogs.com/reports/?fight=last')).toThrow(
