...
export const parseReportUrl = (url: string): ParsedReportUrl => {
  ...
  const queryReportCode =
    parsedUrl.searchParams.get('report') ?? parsedUrl.searchParams.get('code');
  const reportCode = queryReportCode?.trim() || extractReportCodeFromPath(parsedUrl.pathname);

  if (!reportCode || !REPORT_CODE_PATTERN.test(reportCode)) {
    throw new Error('Could not find a Warcraft Logs report code in the URL');
  }
