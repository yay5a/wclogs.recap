import type { NormalizedBossPerformance, NormalizedLeaderboardEntry } from './index.js';
...
export const parseFightSummaries = (report: Record<string, unknown>): FightSummaryRow[] =>
  (Array.isArray(report.fights) ? report.fights : []).flatMap((value) => {
    ...
    const dungeonPulls = (Array.isArray(fight.dungeonPulls) ? fight.dungeonPulls : []).flatMap(
      (pullValue) => {
    ...
    const inProgress = typeof fight.inProgress === 'boolean' ? fight.inProgress : undefined;
    ...
    return [
      {
        ...
        ...(typeof originalEncounterID === 'number' ? { originalEncounterID } : {}),
...
export const sumTableValues = (entries?: readonly TableValueEntry[]): number | undefined => {
