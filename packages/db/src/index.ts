...
const isRecapSummary = (value: unknown): value is RecapSummary => {
  ...
  const isGameFamily = raw.gameFamily === 'retail' || raw.gameFamily === 'mop_classic';
  const isCompareMode = raw.compareModeUsed === 'character' || raw.compareModeUsed === 'mixed';
  const isVisibility =
  ...
  const isCoachingShareability =
    raw.coachingShareability === 'private' || raw.coachingShareability === 'shareable';
  const isRecapPostMode =
    raw.recapPostMode === 'preview-and-post' || raw.recapPostMode === 'preview-only';
...
const toRecapPreviewStateRecord = (doc: unknown): RecapPreviewStateRecord | null => {
...
export class MongoCoachingViewService implements CoachingViewService {
  public async buildShareableCoachingView(reportCode: string): Promise<unknown> {
...
export class MongoAccountabilityViewService implements AccountabilityViewService {
...
export class MongoTrendTrackingService implements TrendTrackingService {
  public async ingestRaidHistory(guildId: string, report: NormalizedReport): Promise<void> {
  ...
  public async recomputeTrendsForGuild(guildId: string): Promise<void> {
    ...
    for (const summary of summaries) {
      ...
      const profileId = rawProfileId instanceof mongoose.Types.ObjectId ? rawProfileId : undefined;

      const name =
        typeof summary.characterName === 'string' && summary.characterName.trim().length > 0
    ...
    type TrendBulkWriteOperations = NonNullable<Parameters<typeof TrendSnapshotModel.bulkWrite>[0]>;
