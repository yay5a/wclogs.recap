...
const makeApp = async (
  ...
  const routeOptions = {
    ...
    ...(options.onboardingStore !== undefined ? { onboardingStore: options.onboardingStore } : {}),
...
describe('dashboard auth', () => {
  ...
  it('rejects missing Discord OAuth state before token exchange', async () => {
    ...
    const callback = await app.inject('/api/dashboard/discord/callback?code=abc&state=missing');
...
describe('dashboard guild routes', () => {
  ...
  it('rejects and revokes claims by stable claim ID', async () => {
    ...
    expect(
      revoke.json<{ claim: { claimId: string; status: string; revokeReason: string } }>().claim,
    ).toMatchObject({
      claimId: revokedClaim.claimId,
      status: 'revoked',
      revokeReason: 'duplicate_wrong_character',
