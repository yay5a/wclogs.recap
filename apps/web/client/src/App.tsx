...
import {
  StaleGuildRequestError,
  useGuildRequestGuards,
} from './dashboard/useGuildRequestGuards.js';
...
export const App = () => {
  ...
  const handleCreateGuild = (event: SyntheticEvent<HTMLFormElement>) => {
    event.preventDefault();
  ...
  const handleSaveConfig = (event: SyntheticEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!config) return;
  ...
  const handleAddOfficer = (event: SyntheticEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!config) return;
    const guildId = config.guildId;
    ...
    <main className="dashboard-shell">
      <aside className="sidebar">
        <div className="brand">
          <p className="eyebrow">wclogs.report</p>
          <h1>Operations</h1>
          <small>{auth.kind === 'discord' ? auth.displayName : 'Admin secret'}</small>
          <LegalLinkStrip />
        </div>
        ...
        <div className="guild-list">
          {guilds.map((guild) => (
            ...
            >
              ...
              <small>
                {guild.compareOfficerUserCount} officers - {guild.autoReportChannelCount} channels
              </small>
            </button>
          ))}
        </div>
      ...
      <section className="workspace">
        ...
        {!config ? (
          ...
          <>
            ...
            {tab === 'activity' ? <ActivityTab activity={activity} directory={directory} /> : null}
          </>
        )}
      </section>
