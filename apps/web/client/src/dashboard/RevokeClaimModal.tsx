...
export const RevokeClaimModal = ({
    ...
    const selectedReasonLabel =
        revokeReasons.find(([value]) => value === reason)?.[1] ?? reason;
    ...
    return (
        <div className="modal-backdrop">
            <form className="modal" onSubmit={onSubmit}>
                <h3>Revoke claim?</h3>
                <p>
                    This will revoke {userLabelForClaim(claim, directory)}'s claim for{" "}
                    {claimCharacter(claim)}.
                </p>
                <p>Reason: {selectedReasonLabel}</p>
                <p>
                    The character will be removed from the approved claims list. Historical
                    claim/activity records will not be deleted.
                </p>
                <label>
                    Reason
