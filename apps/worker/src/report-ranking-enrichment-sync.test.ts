...
describe('syncReportRankingEnrichment', () => {
  ...
  it('keeps scanning fresh reports until the enrichment cap is reached', async () => {
    const wclClient = makeWclClient();
    const store = makeStore();
    const freshReports = Array.from({ length: 2 }, (_, index) => {
      const reportCode = `FRESH${index + 1}`;
      const hashes = getReportRankingEnrichmentQueryHashes({
        reportCode,
        fights: [
          {
            fightId: 11,
      ...
      return {
        reportCode,
        payloads: hashes.map((queryVarsHash) => ({
          queryVarsHash,
          latestFetchedAt: new Date('2026-05-15T12:00:00.000Z'),
        })),
      };
    });
    store.getRawStates.mockResolvedValue(freshReports);
    ...
    const result = await syncReportRankingEnrichment({
      ...
      scope,
      reports: [
        { reportCode: 'FRESH1', startTime: 300 },
        { reportCode: 'FRESH2', startTime: 200 },
        { reportCode: 'STALE', startTime: 100 },
      ],
      timeframes: ['today'],
      compareModes: ['rankings'],
      maxReports: 1,
    ...
    expect(wclClient.fetchReportIndex).toHaveBeenCalledTimes(3);
    expect(wclClient.fetchReportRankingEnrichment).toHaveBeenCalledTimes(1);
    expect(wclClient.fetchReportRankingEnrichment).toHaveBeenCalledWith(
